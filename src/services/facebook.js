...
async function getPostsByPageId(pageId) {
  ...
  const posts = await page.evaluate(async () => {
    return Array.from(
        document.querySelectorAll('[data-testid="post_message"]')
    ).map(post => {
      const header = post.previousElementSibling;
      const time = header
          .querySelector("[data-utime]")
          .getAttribute("data-utime");
      const exposedLink = post.querySelector(".text_exposed_link a");

      // Expanding post
      [...post.querySelectorAll(".text_exposed_show")].forEach(el =>
          el.classList.remove("text_exposed_show")
      );
      ...
      return {
        ...
        exposedLink:
            exposedLink && !exposedLink.classList.contains("see_more_link")
                ? exposedLink.getAttribute("href")
                : null
      };
    });
  });
